...
function ReplyComment(props) {
    ...
    const [ChileCommentNumber, setChileCommentNumber] = useState(0)
    ...
    useEffect(() => {
        ...
        props.commentLists.map((comment) => {
            ...
            setChileCommentNumber(commentNumber)
        })
    }, [props.commentLists])
    ...
    return (
        <div>
            {
                ChileCommentNumber > 0 &&
                <p style={{fontSize: '14px', margin: 0, color: 'gray'}} onClick={onHandleChange}>
                    View {ChileCommentNumber} more comment(s)
                </p>
            }
